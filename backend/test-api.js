...
async function runTests() {
  console.log('🧪 Starting API Endpoints & RBAC Authentication Test Suite...');
  ...
  try {
    ...
    console.assert(loginRes.status === 200, 'Auth login with PIN 8888 should return 200');
    console.assert(loginRes.data.user.role === 'admin', 'Role should be admin');
    console.log('✅ POST /api/auth/login passed (PIN 8888 -> Super Admin)');
    ...
    // 6. POST new talent (With Admin Role Header)
    const newTalentRes = await request('/api/talents', {
      ...
      body: {
        ...
        rate_card: { standard: 'Rp 35.000.000', premium: 'Rp 60.000.000' },
        status: 'available'
      }
    });
    ...
    // 7. Unauthenticated/Guest RBAC rejection check
    const guestPostRes = await request('/api/talents', {
      method: 'POST',
      headers: { 'x-user-role': 'guest' },
      body: { name: 'Unauthorized Talent', title: 'Hacker', category: 'Model' }
    });
    console.assert(guestPostRes.status === 403, 'Guest user should receive 403 Forbidden on POST /api/talents');
    console.log('✅ RBAC Guard Test Passed: Guest blocked with 403 Forbidden');
    ...
    const newProjectRes = await request('/api/projects', {
      ...
      body: {
        ...
        budget_range: 'Rp 100.000.000 - Rp 250.000.000',
    ...
    // 16. Clean up created project & talent
    ...
    console.log('\n🎉 ALL RBAC & API ENDPOINT TEST SUITE ASSERTIONS PASSED SUCCESSFULLY!');
  } finally {
    server.close();
  }
}
